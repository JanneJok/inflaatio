...
import { moneyCalculator, yearsTable, PATH as MONEY_FI, PATH_EN as MONEY_EN } from './rahanarvo.js';
import { calcJsonLd, sourceLineEn } from './laskurit.js';
...
function moreLinks(ctx, exclude) {
  ...
  return ctx.c.cardGrid(cards.filter((c) => c.href !== exclude));
}
...
async function rentPage(ctx) {
  ...
  <dl class="calc-terms">
    <div><dt>Elinkustannusindeksi (cost-of-living index, 1951:10=100)</dt><dd>A long price index series of Statistics Finland with October 1951 = 100. It moves with the consumer price index and is the index most leases refer to. It was ${F.idx(pointAt(ekiSeries, lastEki), 0)} in ${F.period(lastEki)}.</dd></div>
...
async function moneyPage(ctx) {
  ...
  <p>The result tells how much money buys the same basket of goods and services. It does not tell how incomes or living standards changed.</p>
</div>`;
...
${c.section({ id: 'more', title: 'More in English', body: moreLinks(ctx, MONEY_EN) })}`;
  ...
  const description = `What is an old amount worth today? €100 in January 2000 = ${F.eur(d.value)} in ${F.period(latestMonth)}. Official Finnish price indices, markka too.`;
  return {
    path: MONEY_EN,
    changefreq: 'monthly',
    html: ctx.layout({
